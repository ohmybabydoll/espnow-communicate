...
{
    audio_queue = xQueueCreate(30, sizeof(esp_now_data_packet));
    if (audio_queue == NULL)
    {
...
static void example_espnow_recv_cb(const esp_now_recv_info_t *recv_info, const uint8_t *data, int len)
{
    ...
    else if (MY_CODE == recv_data->code)
    {
        ...
        // }
        if (xQueueSend(audio_queue, recv_data, QUEUE_RECV_DELAY) != pdTRUE)
        {
            ESP_LOGE(TAG, "Queue send failed");
        }
        // i2s_write_max98357(output_16bit, len_16bit);
    }
    else
    {
...
{
    esp_now_data_packet recv_data;
    int len_16bit = ESPNOW_DATA_SIZE / 2;

    while (1)
    {
        if (xQueueReceive(audio_queue, &recv_data, portMAX_DELAY) == pdTRUE)
        {
            for (int i = 0; i < len_16bit; i++)
            {
                output_16bit[i] = (recv_data.data[i * 2] << 8) | recv_data.data[i * 2 + 1];
            }
            i2s_write_max98357(output_16bit, ESPNOW_DATA_SIZE / 2);
        } else {
...
//     ESP_LOGI(TAG, "test send sine wave ");
...
void espnow_global_init(void)
{
    ...
    xTaskCreate(write_audio_queue, "write_audio_queue", 1024, NULL, 4, NULL);

    // Check if memory allocation was successful
    if (output_16bit == NULL)
    {
